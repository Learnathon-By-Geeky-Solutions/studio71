        ...
        if (obstacles.Length > 0)
        {
            ...
            foreach (Collider obstacle in obstacles)
            {
                ...
                if (distanceToObstacle < closestDistance)
                {
                    ...
                    if (Physics.Raycast(obstacle.transform.position, obstacleToPlayer.normalized, obstacleToPlayer.magnitude, obstacleLayer))
                    {
    ...
    public Vector3 FindPositionWithLineOfSight()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (NavMesh.SamplePosition(potentialPosition, out NavMeshHit hit, 2f, NavMesh.AllAreas))
            {
                ...
                if (!Physics.Raycast(hit.position, directionToPlayer.normalized, directionToPlayer.magnitude, obstacleLayer))
                {
                    return hit.position;
                }
            }
